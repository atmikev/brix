...
0. **Pick the mode** — Review mode when the subject is a change: a diff, uncommitted work, a branch, a PR, or "what you just did". Explain mode when the subject is existing code. If genuinely ambiguous ("walk me through auth" right after auth changes landed), ask. Then dispatch all of these in a **single response**, so the scout runs while the user answers:
   ...
   - **Scout (Agent, `run_in_background: true`):** Explain mode: read `docs/scan.md` and dispatch the `SMALL` scout with Deep Dive file targets (8-15 files) — if the user then picks Overview, use the most important subset. Review mode: read `docs/review.md` — resolve the changeset inline first (same response, before the Agent call), then dispatch the review scout as a `MEDIUM` sub-agent (it maps the diff AND the surrounding feature code, and judges what the change missed). Either mode: if the optional `graphify` CLI is installed and `graphify-out/graph.json` exists, run a `graphify query` inline first and seed the scout with the result; otherwise scout from scratch, silently (see the scout docs). No highlights yet — discovery only.
...
1. **Collect scout output** — the scout was dispatched in step 0 and often finishes while the user is still answering. Wait for its result before planning; don't re-dispatch.
2. **Plan + generate** — Two paths depending on depth:
   ...
   - **Deep Dive / Thorough review** — Read `docs/plan.md` (and `docs/review.md` for review-mode ordering rules). Dispatch `LARGE` planner to build narrative + transition objects. Then read `docs/segments.md` and dispatch parallel `MEDIUM` segment agents (all at once, in the background) **in the same response that shows the plan outline** — generation overlaps the user's approval pause; see `docs/plan.md`. Create a unique temp dir with `mktemp -d` and have each agent write its segment there. Wait for ALL agents to complete, then assemble from files with `jq` and send one full `set_plan`. Clean up the temp dir after sending. Do NOT send anything to the sidebar until everything is ready.
...
4. **Wrap up** — Explain mode: 3-5 key takeaways, how the feature fits the broader architecture, offer to dive deeper or explain related features. Review mode: the fit-check verdict from `docs/review.md` (does the change fit the surrounding code? missed call sites? convention drift?), then any decisions the user still owes. Either mode: if the scout ran without a graphify graph but the `graphify` CLI is installed, you may offer ONCE — "want me to build a knowledge graph of this repo now (`graphify .`, takes a few minutes, uses LLM calls) so future scouting is near-instant?" — and run it yourself if they say yes. If the CLI isn't installed, mention `uv tool install graphifyy` instead. Never auto-build without asking, never repeat the offer if they don't bite.
...
| Sidebar check not parallelized | Dispatch Bash health check + background scout + AskUserQuestion in one response, not sequentially |
| Idle during human pauses | The scout runs while the user answers preferences; segment agents run while the plan outline awaits "go". Never sit blocked on input with no agents working |
